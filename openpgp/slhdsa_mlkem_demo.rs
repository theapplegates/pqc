...
fn main() -> openpgp::Result<()> {
    ...
    let slhdsa_supported = openpgp::crypto::backend::interface::Asymmetric::supports_algo(
        PublicKeyAlgorithm::SLHDSA256s
    );
    let mlkem_supported = openpgp::crypto::backend::interface::Asymmetric::supports_algo(
        PublicKeyAlgorithm::MLKEM1024_X448
    );
    
    println!("  ✓ SLH-DSA-256s: {}", if slhdsa_supported { "YES" } else { "NO" });
    println!("  ✓ ML-KEM-1024+X448: {}", if mlkem_supported { "YES" } else { "NO" });
    println!("  ✓ SHA3-512: YES (automatic with V6)\n");
    
    if !slhdsa_supported || !mlkem_supported {
        eprintln!("ERROR: Required algorithms not supported!");
        eprintln!("Make sure you built with: --no-default-features --features crypto-openssl,compression");
        std::process::exit(1);
    }
    ...
    let (cert, _revocation) = CertBuilder::new()
        ...
        .set_packet_version(PacketVersion::V6)
        // User ID
        ...
        // Generate primary with SLH-DSA-256s
    ...
    println!("  Packet version: V{}", match cert.primary_key().version() {
        6 => "6 ✓",
        v => panic!("Wrong version: V{}", v),
    });
    
    println!("  Primary key: {:?}", cert.primary_key().pk_algo());
    assert_eq!(cert.primary_key().pk_algo(), PublicKeyAlgorithm::SLHDSA256s,
               "Primary key must be SLH-DSA-256s");
    
    // Check subkeys
    let subkeys: Vec<_> = cert.keys().subkeys().collect();
    ...
    // Step 3: Cleartext sign a message (SHA3-512 hash)
    ...
    let mut signed_message = Vec::new();
    {
        ...
        let mut signer = Signer::new(message_writer, signing_keypair)
            ...
            .hash_algo(HashAlgorithm::SHA3_512)  // Explicit SHA3-512
            .build()?;
    ...
    impl<'a> DecryptionHelper for DecHelper<'a> {
        fn decrypt<D>(&mut self, pkesks: &[openpgp::packet::PKESK],
                      _skesks: &[openpgp::packet::SKESK],
                      sym_algo: Option<SymmetricAlgorithm>,
                      mut decrypt: D)
                      -> openpgp::Result<Option<openpgp::Fingerprint>>
        where D: FnMut(SymmetricAlgorithm, &openpgp::crypto::SessionKey)
                       -> bool
        {
            for pkesk in pkesks {
                let mut keypairs = self.cert.keys().with_policy(self.policy, None)
                    .for_transport_encryption()
                    .for_storage_encryption()
                    .secret()
